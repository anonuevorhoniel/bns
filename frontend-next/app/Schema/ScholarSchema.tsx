...
export const scholarResolver = z.object({
    //personal info
    first_name: z
        .string(requireError("First Name"))
        .min(2, { error: "First name should be atleast 2 characters" }),
    middle_name: z.string(requireError("Middle Name")).nullable().optional(),
    last_name: z
    ...
    civil_status: z.string(requireError("Civil Status")).nullable().optional(),
    contact_number: z
        .string()
        .nullable()
        .optional()
    ...
    status: z.preprocess(
        (val) => val ?? "",
        z.string(requireError("Status")).min(1, { error: "Status is required" })
    ),
