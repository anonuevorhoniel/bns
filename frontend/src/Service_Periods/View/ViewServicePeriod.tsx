...
export default function ViewServicePeriod() {
    const { viewOpen, setViewOpen, setDataView, scholarId } =
        UseViewServicePeriod();
    const { data } = useQuery({
        queryKey: ["viewServicePeriod", scholarId],
        queryFn: () => setDataView(),
        refetchOnWindowFocus: false,
        enabled: !!scholarId,
    });
    const dataView = data?.dataView;
    const viewPageInfo = data?.viewPageInfo;
